...
// Property 19a: For any diff with fewer than threshold lines, the full diff is returned
// Property 19b: For any diff with threshold or more lines, a summarized version is returned
...
// TestPropertySmallDiffsSentInFull verifies that diffs below the threshold are sent in full.
...
func TestPropertySmallDiffsSentInFull(t *testing.T) {
	...
	properties.Property("diffs under threshold are sent in full", prop.ForAll(
		func(lineCount int) bool {
			...
			// Ensure we're under threshold (lineCount is already constrained by generator)
		...
		gen.IntRange(1, DiffThreshold-1), // lineCount under threshold
	))

	properties.TestingRun(t)
}

// TestPropertyLargeDiffsSummarized verifies that diffs at or above the threshold are summarized.
...
func TestPropertyLargeDiffsSummarized(t *testing.T) {
	...
	properties.Property("diffs at or above threshold are summarized", prop.ForAll(
		func(lineCount int, fileCount int) bool {
			...
			if !result.IsSummarized {
				t.Logf("FAILED: diff with %d lines was not summarized (threshold: %d)", lineCount, DiffThreshold)
				return false
			}

			// Property: Summary MUST be different from original (shorter)
			if result.Content == diff {
				t.Logf("FAILED: summary is identical to original diff for %d lines", lineCount)
				return false
			}
		...
		gen.IntRange(DiffThreshold, DiffThreshold+500), // lineCount at or above threshold
	...
	properties.Property("threshold boundary is correctly respected", prop.ForAll(
		func(_ int) bool {
			...
			if !atResult.IsSummarized {
				t.Logf("FAILED: diff with %d lines (threshold) was not summarized", atActual)
				return false
			}
...
func TestPropertySummaryPreservesFileInfo(t *testing.T) {
	...
	properties.Property("summary contains file-level information", prop.ForAll(
		...
		gen.IntRange(DiffThreshold, DiffThreshold+200), // lineCount
...
func TestPropertyCustomThreshold(t *testing.T) {
	...
	properties.Property("custom threshold is respected", prop.ForAll(
		func(threshold int, lineCount int, fileCount int) bool {
			...
			// Property: Summarization decision MUST be based on custom threshold
